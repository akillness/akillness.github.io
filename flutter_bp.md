---
layout: page
title: Flutter 앱 개발일지
subtitle: Flutter 를 공부하며 개발하기 -초급편-
---

{: .box-success}
Task Planning 을 통해, Flutter의 사용방법 및 앱 개발을 위한 전반적인 사전 지식을 습득하고 앱 개발에 대한 감각을 익히는 것이 중점입니다. 해당 Task는 자발적인 학습을 통해 Flutter 의 개발과 관련된 본인의 지식을 공유하고 함께 코드를 자유롭게 수정,개발,배포할 수 있는 적정 수준까지 끌어올리는 것을 목표로 합니다.


### Flutter를 이용한 앱 개발을 위한 Task Planning:

| 카테고리 | 일정 |항목 | 세부항목 | 
| :-------:| :-------: |:---: | :---: |
| 개념 |  | [Flutter의 개념 및 핵심 구성요소 확인](#flutter의-개념-및-핵심-구성요소-확인) | :arrow_right: 기본: Dart 프로그래밍 |
| 동기 |  | [Flutter를 이용해 개발하는 이유](#flutter를-이용해-개발하는-이유) | :arrow_right: 사례: 네이버지식iN |
| 개발 |  | [개발환경설정](#개발환경설정) | - |
| 개발 |  | [Flutter 를 이용한 app 기획 - 1 시계 만들기](#flutter를-이용한-app-기획-1) | - |
| 개발 |  | [app 세부구성(1)](#app-세부구성(1)) | - |
| 개발 |  | [app 기능구현(1)](#app-기능구현(1)) | - |
| 개발 |  | [Flutter 를 이용한 app 기획 - 2 블로그 만들기](#flutter를-이용한-app-기획-2) | - |
| 개발 |  | [app 세부구성(2)](#app-세부구성(2)) | - |
| 개발 |  | [app 기능구현(2)](#app-기능구현(2)) | - |
| 개발 |  | [DB 만들기](#db-만들기) | - |
| 개발 |  | [마켓플레이스에 올려보기](#마켓플레이스에-올려보기) | - |


> :warning: 아래의 상세 내용은 아직 작성중에 있습니다.
***

## Flutter의 개념 및 핵심 구성요소 확인

## Flutter를 이용해 개발하는 이유

[This is a link to a different site](https://deanattali.com/) and [this is a link to a section inside this page](#local-urls).

## 개발환경설정

<!-- ![Crepe](https://beautifuljekyll.com/assets/img/crepe.jpg) -->

here is the same code with syntax highlighting:

```javascript
var foo = function(x) {
  return(x + 5);
}
foo(3)
```

## Flutter를 이용한 app 기획 1
You can add notification, warning and error boxes like this:
{: .box-note}
**Note:** This is a notification box.

## app 세부구성(1)

## app 기능구현(1)

## Flutter를 이용한 app 기획 2
You can add notification, warning and error boxes like this:
{: .box-warning}
**Warning:** This is a warning box.
## app 세부구성(2)

## app 기능구현(2)

## DB 만들기
You can add notification, warning and error boxes like this:
{: .box-error}
**Error:** This is an error box.

## 마켓플레이스에 올려보기

{: .box-note}
**Note:** This is a notification box.


### Reference 
- [https://brunch.co.kr/@tilltue/56](https://brunch.co.kr/@tilltue/56)
- [https://d2.naver.com/helloworld/3384599](https://d2.naver.com/helloworld/3384599)
- [https://flgichacha.tistory.com/2](https://flgichacha.tistory.com/2)
- [https://flutter.ducafecat.com/ko/pubs/html_editor-package-info](https://flutter.ducafecat.com/ko/pubs/html_editor-package-info)
- [https://yozm.wishket.com/magazine/detail/1566/](https://yozm.wishket.com/magazine/detail/1566/)


<!-- ## Local URLs in project sites {#local-urls} -->
<!-- ![Crepe](/assets/img/crepe.jpg) -->
