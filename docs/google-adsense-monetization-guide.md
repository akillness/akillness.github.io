...
5. 이번 개선은 미완성 Flutter 페이지를 은퇴하고, 800단어 미만 XOR 메모를 검색·사이트맵·
   광고에서 제외하며, 같은 누락을 CI에서 막는 규칙을 추가한 것이다. 라이브 URL과 사이트맵,
   GitHub Actions 성공을 확인한 뒤 **2026-09-24 00:19 KST에 검토 요청을 제출**했고,
   현재 화면 상태는 `준비 중 / 리뷰가 요청됨`이다. 결과가 나올 때까지 재요청하지 않는다.
...
`site approved` 는 구글 심사가 끝나야 통과하며, `ads.txt seen by AdSense`는 현재
`승인됨`이다. 검토 요청 직후에는 사이트 상태가 `준비 중 / 리뷰가 요청됨`인 것이 정상이다.
