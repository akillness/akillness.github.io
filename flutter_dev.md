...
네이버지식iN 개발 사례를 통해, 엡 개발에 Flutter를 사용한 이유의 필요성을 파악해봅시다.
...
+ 서비스상 개발자 불균형 문제
  +  각각 별도의 플랫폼을 개발하기 때문에 개발, 테스트, 배포를 플랫폼 별로 일진행하기 때문에 한 플랫폼의 개발자가 부족하면 스펙 구현이 불가능해지거나 배포 일정을 맞출 수 없게 됩니다. 
+ 개발 효율성 문제 
  + 기술공유
    + 각각의 플랫폼 커뮤니티만 활성화
  + 네이밍 컨벤션 통일
