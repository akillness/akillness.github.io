...
**For production retrieval in 2026, start from a maintained family.** Qwen3-Embedding, EmbeddingGemma, bge-m3 and multilingual-e5 all have active cards and millions of monthly downloads. Reach for LLM2Vec when you specifically want to convert a particular backbone you already run, and accept the dependency pin. If you are choosing an embedding stack today, the [multimodal Sentence Transformers guide](/posts/multimodal-sentence-transformers-practical-guide/) covers the current model families and their trade-offs.
